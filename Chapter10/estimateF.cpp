...
int main()
{
    ...
    cv::drawMatches(image1, keypoints1,         // 1st image and its keypoints
                    ...
                    matches,                    // the selected matches (selMatches) or the all matches  (matches)
    ...
    cv::Mat fundamental = cv::findFundamentalMat(selPoints1,      // points in first image
    ...
    cv::computeCorrespondEpilines(selPoints1,  // image points
    ...
    for (std::vector<cv::Vec3f>::const_iterator it = lines2.begin(); it != lines2.end(); ++it) {
        ...
        cv::line(image1, cv::Point(0, -(*it)[2] / (*it)[1]),
                 cv::Point(image1.cols, -((*it)[2] + (*it)[0] * image1.cols) / (*it)[1]), cv::Scalar(255, 255, 255));
    }
